...
#****************************************************************************
def rotate_ar_2d(_data, _allrange, _ang): #AH04092019
    """
    Reshape 1D data to 2D array (image) according to the range definition in _allrange; 
    Then rotate the 2D array (image) according to the rotation angle _ang;
    Then reshape the rotated 2D array (image) back to 1D array according to the new range in allrange;
    :param _data: 1D data array
    ...
    :return data: 1D array with the rotation data; 
            allrange: the new range information for the rotated 2D array (image)
    """
    allrange =list(_allrange)
    data = np.reshape(_data, (_allrange[8], _allrange[5]), order='C')
    print('Before Rotation Size: {}, Dimensions: {}'.format(data.size, data.shape))
    shape_before = list(data.shape)

    from scipy.ndimage import rotate 
    data = rotate(data, _ang, reshape = True, mode = 'constant', order = 3)

    print('After Rotate Size: {}, Dimensions: {}'.format(data.size, data.shape))
    shape_after = list(data.shape)

    print('x_range and y_range before rotate is [{}, {}] and [{}, {}]'.format(allrange[3], allrange[4], allrange[6], allrange[7]))
    allrange[3] = shape_after[0]/float(shape_before[0])*allrange[3]
    allrange[4] = shape_after[0]/float(shape_before[0])*allrange[4]
    allrange[6] = shape_after[1]/float(shape_before[1])*allrange[6]
    allrange[7] = shape_after[1]/float(shape_before[1])*allrange[7]
    print('x_range and y_range after rotate is [{}, {}] and [{}, {}]'.format(allrange[3], allrange[4], allrange[6], allrange[7]))
    ...
    return data, allrange
#****************************************************************************
def xcut_ar_2d(_data, _x_range, _y_range, _yc): #AH04092019
    """
    Function to get the 2D array/image cutted data along line 'y=_yc'
    : param _data: 1D array / image
    : param _x_range: list with 3 values: [x0, x1, nx]
    : param _y_range: list with 3 values: [y0, y1, ny]
    : param _yc: 
    : return 1D array with size nx
    """    
    x0 = _x_range[0]; x1 = _x_range[1]; nx = _x_range[2]
    y0 = _y_range[0]; y1 = _y_range[1]; ny = _y_range[2]
    xStep = 0
    ...
    xx = x0
    for ix in range(nx):
        arCutX[ix] = interp_2d(xx, _yc, x0, xStep, nx, y0, yStep, ny, _data, inperpOrd, 1, 0)
        xx += xStep
    return arCutX

...
class Backend(object):
    ...
    def uti_plot2d(self, ar2d, x_range, y_range, colormap, labels=('Horizontal Position [m]','Vertical Position [m]')): #AH29082019
        #fig = _pl.figure(figsize=(12,8))
        fig = self._pl.figure()
        self._plot_2D(ar2d,x_range,y_range,labels,fig, colormap) #AH29082019
        return self._maybe_savefig(fig)

    def uti_plot2d1d(self, data, x_range, y_range, xc, yc, labels, colormap, _graphs_joined=True): #AH29082019
        ...
        if _graphs_joined:
            ...
            self._plot_2D(data, x_range, y_range, label2D, fig, colormap, 131) #showing graphs in one panel #AH29082019
        else: self.uti_plot2d(data, x_range, y_range, colormap, label2D) #AH29082019
    ...
    def uti_plot_data_file(self, _fname, _read_labels=1, _e=0, _x=0, _y=0, _graphs_joined=1, #OC16112017 (renamed: uti_plot_data_file -> uti_plot_data_file)
                           ...
                           _scale='linear', _width_pixels=None, _rotation_angle=0, _colormap='Greys_r'):  #AH28082019
        ...
        if _rotation_angle: # when _rotation_angle is true, then rotate the image data according to angle. #AH04092019
            try:
                ...
                print('Cannot rotate the image - scipy.ndimage.rotate() cannot be imported. ')
        ...
        if mode==m.T:
            ...
            fig = self.__mode_HV(data, allrange, arLabels, arUnits, _x, _y, _colormap, _graphs_joined) #AH29082019
        #elif mode==m.EV:
        #  fig = __mode_EV(data,allrange)
        #elif mode==m.EH:
        #  fig = __mode_EH(data,allrange)
        elif mode==m.EHV:
            fig = self.__mode_EHV(data, allrange, arLabels, arUnits, _e, _x, _y, _colormap, _graphs_joined) #AH29082019
...
#        return self.uti_plot_data_file(_fname, _read_labels, _e, _x, _y, _graphs_joined, _multicolumn_data, _column_x, _column_y, _scale, _width_pixels) #OC16112017
    ...
    def _plot_2D(self, ar2d, x_range, y_range, labels, fig, colormap, typ=111): #AH29082019
        ...
        ax.pcolormesh(x,y,ar2d,cmap=colormap) #AH29082019
    ...
    def __mode_HV(self, data, allrange, _ar_labels, _ar_units, _xc=0, _yc=0, _colormap='Greys_r', _graphs_joined=True): #AH29082019
        ...
        return self.uti_plot2d1d(data, range_x, range_y, xc, yc, [label2D, label1H, label1V], _colormap, _graphs_joined) #AH29082019
    ...
    def __mode_EHV(self, data, allrange, _ar_labels, _ar_units, _ec, _xc, _yc, _colormap, _graphs_joined=1): #AH29082019
        #allrange, units = _rescale_range(allrange)
        #allrange, units = _rescale_range(allrange, _ar_units, _ec, _xc, _yc)
        allrange, units = uti_plot_com.rescale_range(allrange, _ar_units, _ec, _xc, _yc)
        ...
        if _graphs_joined:
            ...
            self._plot_2D(arCutXY, range_x, range_y, label2D, fig, colormap, 221) #showing graphs in one figure #AH29082019
            ...
            self.uti_plot2d(arCutXY, range_x, range_y, colormap, label2D) #AH29082019
